...
using PhccHsm.ConfigManager.UI.DigitalConfig;
using PhccHsm.ConfigManager.UI.Doa7SegConfig;
using PhccHsm.ConfigManager.UI.Servo;
using PhccHsm.Phcc;
...
namespace PhccHsm.ConfigManager.UI
{
    public partial class frmPhccDeviceManager : Form
    {
        ...
        private List<PhccHardwareSupportModule> _hsm;
        #endregion
        ...
        private void RefreshPhccHsm()
        {
            if (_hsm == null)
            {
                _hsm = new List<PhccHardwareSupportModule>();
                foreach (var mb in _configMgr.Motherboards)
                {
                    _hsm.Add(PhccHardwareSupportModule.GetModuleForCalibration(mb));
                }
