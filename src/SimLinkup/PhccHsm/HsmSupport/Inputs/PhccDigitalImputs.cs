...
namespace PhccHsm.HsmSupport.Inputs
{
    public class PhccDigitalImputs
    {
        ...
        public void InitializeSignals(object device, List<DigitalConfig> inputConfig)
        {
            ...
            for (var i = 0; i < 1024; i++)
            {
                ...
                if (_inputConfig.Any(x => x.PinNumber == i + 1))
                {
                    ...
                    {
                        Category = "PHCC Inverted Inputs",
                        CollectionName = "Digital Inputs",
                        FriendlyName = $"Digital Inverted Input {string.Format($"{i + 1:0}", i + 1)}",
                        Id = $"PhccDigitalInvertedInput[{_portName}][{i}]",
                        Index = i,
        ...
        private void device_DigitalInputChanged(object sender, DigitalInputChangedEventArgs e)
        {
            ...
            var signals = DigitalInputs.Where(c => c.Index == e.Index).ToList();
            foreach (var signal in signals)
            {
                if (signal.FriendlyName.Contains("nverted"))
