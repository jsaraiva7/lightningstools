...
namespace Solid.Arduino
{
    ...
    public class SerialConnection : SerialPort, ISerialConnection
    {
        ...
        /// Finds a serial connection to a device supporting the Firmata protocol.
        /// </summary>
        /// <returns>A <see cref="ISerialConnection"/> list, empty if no connection is found</returns>
        /// <remarks>
        /// <para>
        /// This method searches all available serial ports until it finds a working serial connection.
        /// For every available serial port an attempt is made to open a connection at a range of common baudrates.
        ...
        /// <seealso cref="IFirmataProtocol"/>
        /// <seealso href="http://www.firmata.org/wiki/Protocol#Query_Firmware_Name_and_Version">Query Firmware Name and Version</seealso>
        public static List<ISerialConnection> FindAll()
        {
            Func<ArduinoSession, bool> isAvailableFunc = session =>
            {
                Firmware firmware = session.GetFirmware();
                return firmware.MajorVersion >= 1;
            };

            string[] portNames = GetPortNames();
            List<ISerialConnection> connections = FindAllConnections(isAvailableFunc, portNames, PopularBaudRates);
            return connections;
        ...
        private static List<ISerialConnection> FindAllConnections(Func<ArduinoSession, bool> isDeviceAvailable, string[] portNames, SerialBaudRate[] baudRates)
        {
            bool found = false;
            var toRet = new List<EnhancedSerialConnection>();
            ...
            for (int x = portNames.Length - 1; x >= 0; x--)
            {
                foreach (SerialBaudRate rate in baudRates)
                {
                    try
                    {
                        ...
                        if (found)
                            toRet.Add(new EnhancedSerialConnection(portNames[x], rate));
            ...
            return toRet.ToList<ISerialConnection>()  ;
        }
        #endregion
    }
}